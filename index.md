...
> Following the Covid pandemic, the Ever-Given blockage and the Ukrainian war there were stockouts and goods delivery delays. But if prices are flexible, then production cost increases pass through to prices, and there are no stockouts. Only if prices are sticky might firms ration demand through stockouts or delivery delays, to avoid selling goods at a price below marginal cost. However, the standard assumption in solving sticky price models (Calvo or menu-cost), is that firms sell the entire quantity demanded at their price. This paper investigates the consequences of allowing firms to ration under sticky prices, in a continuous time New Keynesian model with idiosyncratic demand shocks. We show rationing helps the model match empirical results from both micro & macro data. Rationing produces a convex, backward bending Phillips curve, yet significantly higher optimal inflation.
